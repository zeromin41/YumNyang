...
function RecipeCard({ imageSrc, title, altText, onClick, recipeId, isLoggedIn, userId }) {
    ...
    // Redux 스토어에서 현재 레시피의 찜 상태와 ID를 가져옴
    const isFavorite = useSelector((state) => selectIsFavoriteByRecipeId(state, String(recipeId)));
    const favoriteEntry = useSelector((state) => selectFavoriteEntryByRecipeId(state, String(recipeId)));
    ...
    const handleHeartClick = useCallback(
        async (e) => {
            ...
            // 사용자에게 확인 메시지 표시
            const confirmMessage = isFavorite
                ? "찜을 취소하시겠습니까?"
                : "찜 하시겠습니까?";

            if (window.confirm(confirmMessage)) { // 사용자가 '확인'을 누른 경우
                setIsLoadingHeart(true);
                try {
                    if (isFavorite && favoriteEntryId) {
                        // 찜 제거
                        await dispatch(removeFavoriteFromServer({ favoriteEntryId, userId: String(userId) })).unwrap();
                    } else {
                        // 찜 추가 
                        if (userId && recipeId) {
                             await dispatch(addFavoriteToServer({ userId: String(userId), recipeId: String(recipeId) })).unwrap();
                        } else {
                            console.error("RecipeCard: userId 또는 recipeId가 유효하지 않아 찜 추가 불가");
